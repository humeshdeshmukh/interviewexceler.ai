...
8. Use Markdown for formatting (bold, lists, etc.) to make your responses easy to read.
...
${externalData.portfolioData ? `Portfolio Overview: ${externalData.portfolioData}` : ''}` : '';
    ...
    const systemInstruction = `${CONSULTANT_PROMPT}
${context}`;
...
export async function generateTitle(content: string): Promise<string> {
  try {
    const model = genAI.getGenerativeModel({ model: 'gemini-2.5-flash-lite' });
    const prompt = `Generate a very short, concise title (max 4-5 words) for a chat that starts with this message: "${content}". Do not use quotes.`;
